...
public class CityStateAIController : MonoBehaviour
{
    ...
    public void UpdateCities()
    {
        foreach (City city in cityState.GetCities())
        {
            ...
            {
                city.BuildingManager.AddBuild(buildConfigs[0]);
            }
    ...
    public IEnumerator UpdateUnit(CombatUnit unit)
    {
        ...
        //if (cityState.CityStateID == 4)
        //{
            nextMove = GetNextMove(unit);

        //nextMove = CheckNearby(unit);
        //if(nextMove)
        //{
        //    nextMove = unit.Behaviour.Attack(nextMove);
        //}
        //if(!nextMove)
        //{
        //    List<City> enemyCities = cityState.GetEnemyCitiesOrderByDistance(unit.HexUnit.Location.coordinates);
        //    if (enemyCities.Count > 0)
        //    {
        //        City city = enemyCities[0];
        ...
        //    if (!nextMove)
        //    {
        //        nextMove = unit.Behaviour.Patrol(cityState.GetCity().GetHexCell(), 4);
        //    }
        //}
        if (!nextMove)
        {
